...
router.post('/select_category', ensureAuthenticated, function(req, res, next){
  ...
  if(!errors){
    Category.getCategoryByName(category, function(err, output_category){
      if(!err && output_category != null){
        User.getUserById(req.user._id, function(err, user){
          if(err) throw err;
          Network.getNetworkById(user.network, function(err, network){
            if(err) throw err;
            res.render('requests/' + output_category.resource, {
              companyName: 'OOO ValeronMan',
              category: category,
              user: user,
              network: network,
              csrfToken: req.csrfToken()
...
router.post('/create_request', ensureAuthenticated, function(req, res, next){
    // console.log('User network: ' + req.user.network);
